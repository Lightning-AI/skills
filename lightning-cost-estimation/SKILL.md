...
so use it when an agent's permission rules block `curl` but allow `lightning`. `lightning api`
always sends credentials, so this route needs `lightning login` or an API key first; only `curl`
works logged out. Once signed in, every `curl` recipe below works with `lightning api "<path>"`
in its place:
