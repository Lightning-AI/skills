...
- **Nothing bounds an unattended run, and `max_runtime` is not a cap.** It maps to a **DWS reservation duration**, honoured only on non-spot machines that are `dws_supported`/`dws_only`; everywhere else it is a no-op that the SDK warns about (`max_runtime has no effect on interruptible (spot) instances`). There is no default, which is why the examples above don't pass it. Three things are easy to confuse:
  - `max_runtime` — asks a scheduler to reserve a window. Ignored on ordinary and interruptible machines.
  - `job.wait(timeout=..., stop_on_timeout=True)` — really does stop the job, but only while the calling process is alive to do it. Kill the shell and the job keeps running and billing.
  - an enforced remote deadline — **does not exist**. Nothing on the platform ends a job for you.

  So never tell a user an overnight run is bounded. If they ask for "stop after N hours" or "spend at most $X", something has to stay alive and call `job.stop()`, and you should say that plainly rather than implying a guarantee.
...
- Image jobs can sit in `Pending`/`creating` for a long time (tens of minutes on busy shared pools) before a machine is scheduled — queue time, before a machine is yours, is not billed. **Provisioning is, and it also reports `Pending`:** once a machine is allocated, billing runs `startedAt`→`stoppedAt`, which covers the image pull (a 397-second pull on a `pytorch/pytorch` image was billed; see `lightning-cost-estimation`). So `Pending` alone doesn't tell you whether the meter is running, and stopping a job mid-provision is not free. Don't treat a slow start as failure. Always use `job.wait(timeout=..., stop_on_timeout=True)` or monitor `job.status` with your own deadline, and `job.stop()`+`job.delete()` if you give up.
