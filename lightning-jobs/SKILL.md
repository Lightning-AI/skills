...
- **`max_runtime` is not a general time or spend cap** — it is a **DWS reservation duration**, applied only on non-spot machines that are `dws_supported`/`dws_only`, and elsewhere a no-op the SDK warns about (`max_runtime has no effect on interruptible (spot) instances`); verified inert on an L40S run. `job.wait(timeout=..., stop_on_timeout=True)` does stop a job, but only while the calling process survives to do it. So don't promise a user an unattended bound on an ordinary machine: say what will actually enforce it, and keep something alive to call `job.stop()`.
